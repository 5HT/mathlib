...
def div (r : α) (s : S) : loc α S := ⟦(r, s)⟧

def of_comm_ring (r : α) : loc α S := div r 1
...
instance of_comm_ring.is_ring_hom : is_ring_hom (of_comm_ring : α → loc α S) :=
{ map_add := λ x y, quotient.sound $ by simp,
  map_mul := λ x y, quotient.sound $ by simp,
  map_one := rfl }

@[simp] lemma of_comm_ring_zero : (of_comm_ring 0 : loc α S) = 0 := rfl
@[simp] lemma of_comm_ring_one : (of_comm_ring 1 : loc α S) = 1 := rfl
@[simp] lemma of_comm_ring_add {x y : α} :
  (of_comm_ring (x + y) : loc α S) = of_comm_ring x + of_comm_ring y := is_ring_hom.map_add _
@[simp] lemma of_comm_ring_sub {x y : α} :
  (of_comm_ring (x - y) : loc α S) = of_comm_ring x - of_comm_ring y := is_ring_hom.map_sub _
@[simp] lemma of_comm_ring_mul {x y : α} :
  (of_comm_ring (x * y) : loc α S) = of_comm_ring x * of_comm_ring y := is_ring_hom.map_mul _
@[simp] lemma of_comm_ring_neg {x : α} :
  (of_comm_ring (-x) : loc α S) = -of_comm_ring x := is_ring_hom.map_neg _
@[simp] lemma of_comm_ring_pow {x : α} {n : ℕ} :
  (of_comm_ring (x ^ n) : loc α S) = of_comm_ring x ^ n := is_semiring_hom.map_pow _ _ _

instance : has_coe α (loc α S) := ⟨of_comm_ring⟩

instance coe.is_ring_hom : is_ring_hom (coe : α → loc α S) :=
localization.of_comm_ring.is_ring_hom

@[simp] lemma coe_zero : ((0 : α) : loc α S) = 0 := rfl
@[simp] lemma coe_one : ((1 : α) : loc α S) = 1 := rfl
@[simp] lemma coe_add {x y : α} : (↑(x + y) : loc α S) = x + y := of_comm_ring_add
@[simp] lemma coe_sub {x y : α} : (↑(x - y) : loc α S) = x - y := of_comm_ring_sub
@[simp] lemma coe_mul {x y : α} : (↑(x * y) : loc α S) = x * y := of_comm_ring_mul
@[simp] lemma coe_neg {x : α} : (↑(-x) : loc α S) = -x := of_comm_ring_neg
@[simp] lemma coe_pow {x : α} {n : ℕ} : (↑(x ^ n) : loc α S) = x ^ n := of_comm_ring_pow

@[simp] protected lemma div_self {x : α} {hx : x ∈ S} :
  (div x ⟨x, hx⟩ : loc α S) = 1 :=
quotient.sound ⟨1, is_submonoid.one_mem S,
by simp only [subtype.coe_mk, is_submonoid.coe_one, mul_one, one_mul, sub_self]⟩

@[simp] lemma div_self' {s : S} :
  (div s s : loc α S) = 1 :=
by cases s; exact localization.div_self

@[simp] lemma div_self'' {s : S} :
  (div s.1 s : loc α S) = 1 :=
div_self'

@[simp] lemma coe_mul_div (x y : α) (s : S) :
  ↑x * div y s = div (x * y) s :=
quotient.sound $ r_of_eq $ by rw one_mul

protected lemma div_eq_mul_one_div (r : α) (s : S) :
  div r s = r * div 1 s :=
by rw [coe_mul_div, mul_one]

@[simp] protected lemma div_mul_div (x y : α) (s t : S) :
  div x s * div y t = div (x * y) (s * t) := rfl

@[simp] protected lemma mul_div_cancel_left (r : α) (s : S) :
  div (↑s * r) s = r :=
by rw [localization.div_eq_mul_one_div, mul_comm ↑s, coe_mul,
       mul_assoc, ← localization.div_eq_mul_one_div, div_self', mul_one]

@[simp] lemma mul_div_cancel_right (r : α) (s : S) :
  div (r * s) s = r :=
by rw [mul_comm, localization.mul_div_cancel_left]

@[elab_as_eliminator]
protected theorem induction_on {C : loc α S → Prop} (x : loc α S)
  (ih : ∀ r s, C (div r s : loc α S)) : C x :=
...
instance rec.is_ring_hom {β : Type v} [comm_ring β]
  (f : α → β) (hf : is_ring_hom f) (g : S → units β) (hg : ∀ s, (g s : β) = f s) :
  is_ring_hom (localization.rec f hf g hg) :=
...
localization.rec f hf (λ s, classical.some hfx ^ classical.some s.2) $ λ s,
...
@[reducible] def quotient_ring := loc α (non_zero_divisors α)
...
def inv_aux (x : β × (non_zero_divisors β)) : quotient_ring β :=
...
@[simp] lemma div_eq_div {r s} : (div r s : quotient_ring β) =
  @has_div.div (quotient_ring β)
    (@division_ring_has_div (quotient_ring β)
      (@field.to_division_ring (quotient_ring β) (@discrete_field.to_field (quotient_ring β) (quotient_ring.field.of_integral_domain β)))
      (@field.to_division_ring (quotient_ring β) (@discrete_field.to_field (quotient_ring β) (quotient_ring.field.of_integral_domain β))))
    r s :=
show _ = _ * dite (s.1 = 0) _ _, by rw [dif_neg (ne_zero_of_mem_non_zero_divisors s.2)];
exact localization.div_eq_mul_one_div _ _
...
mul_one r ▸ coe_mul_div r 1 s ▸ (ideal.mul_mem_right _ $ ideal.mem_map_of_mem $
have _ := @ideal.mul_mem_left (loc α S) _ _ s _ hJ,
by rwa [coe_coe, coe_mul_div, localization.mul_div_cancel_left] at this)

def le_order_embedding :
  ((≤) : ideal (loc α S) → ideal (loc α S) → Prop) ≼o
  ((≤) : ideal α → ideal α → Prop) :=
{ to_fun := λ J, ideal.comap of_comm_ring J,
